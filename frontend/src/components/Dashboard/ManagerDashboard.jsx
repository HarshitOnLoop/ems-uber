...
const ManagerDashboard = () => {
  ...
  const handleDeleteGlobalTask = async (userId, taskId) => {
  ...
  const handleDeleteUserTask = async (taskId) => {
    ...
    try {
      await axios.delete(`${API}/tasks/${taskId}`);
      loadUserTasks(selectedUser._id);
    } catch (err) {
  ...
  const fetchTeams = async () => {
    try {
      ...
      setTeams(res.data);
      if (selectedTeam) {
        const updated = res.data.find(t => t._id === selectedTeam._id);
  ...
  }, [selectedTeam?._id]); // Only re-bind if selected team ID changes

  const handleCreateTeam = async (e) => {
    e.preventDefault();
    if (!teamForm.name) return;
    try {
      await axios.post(`${API}/teams`, { ...teamForm, members: [], tasks: [] });
      setTeamForm({ name: "", description: "" });
      setIsTeamModalOpen(false);
      fetchTeams();
    } catch (err) { console.error("Create team err", err); }
  };
  ...
  const handleDeleteTeam = async (teamId) => {
    if (window.confirm("Delete this team completely?")) {
      try {
        ...
        fetchTeams();
      } catch (err) { console.error("Delete team err", err); }
    }
  };
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      await axios.put(`${API}/teams/${selectedTeam._id}`, { members: updatedMembers });
      setSelectedMemberToAdd("");
      fetchTeams();
    } catch (err) { console.error("Add member err", err); }
  };
  ...
  const handleRemoveMember = async (memberId) => {
    try {
      ...
      await axios.put(`${API}/teams/${selectedTeam._id}`, { members: updatedMembers });
      fetchTeams();
    } catch (err) { console.error("Remove member err", err); }
  };
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      const newTaskObj = { text: newTask, status: "Pending", id: Date.now().toString() };
      const updatedTasks = [...selectedTeam.tasks, newTaskObj];
      await axios.put(`${API}/teams/${selectedTeam._id}`, { tasks: updatedTasks });
      setNewTask("");
      fetchTeams();
    } catch (err) { console.error("Add task err", err); }
  };

  const handleDeleteTask = async (taskId) => {
    try {
      const updatedTasks = selectedTeam.tasks.filter(t => t.id !== taskId && t._id !== taskId);
      await axios.put(`${API}/teams/${selectedTeam._id}`, { tasks: updatedTasks });
      fetchTeams();
    } catch (err) { console.error("Delete task err", err); }
  };

  // ---------------- 5. CHAT LOGIC ----------------
  const handleSendTeamMessage = async (e) => {
    e.preventDefault();
    console.log("Send team message triggered. Input:", teamChatInput.trim(), "Team:", selectedTeam?.name);
  ...
  const handleSendMessage = (e) => {
    e.preventDefault();
    console.log("Send message triggered. Input:", messageInput.trim(), "Recipient:", chatRecipient?.name);

    if (!chatRecipient) {
      alert("Please select an employee to message");
      return;
    }
  ...
  // Load chat history when chatRecipient changes and sync messages periodically
  useEffect(() => {
    if (!chatRecipient) return;
    const allChats = JSON.parse(localStorage.getItem("chat_database") || "{}");
    const userChats = allChats[chatRecipient._id] || [];
    setCurrentChatHistory(userChats);

    // Periodically sync messages to reflect updates from other windows/tabs
  ...
  // Load and sync team chat messages
  useEffect(() => {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.container}>
                ...
                <tbody>
                  {filteredUsers.map((u) => (
                    <tr key={u._id} style={{ backgroundColor: "#fff", borderBottom: "1px solid #e1e4e8" }}>
                      <td style={styles.td}><div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        {getPhotoUrl(u._id) ? (
                          <img src={getPhotoUrl(u._id)} style={{ width: "32px", height: "32px", borderRadius: "50%", border: "1px solid #d0d7de" }} onError={(e) => e.target.style.display = 'none'} />
                        ) : (
                          <div style={{ width: "32px", height: "32px", borderRadius: "50%", border: "1px solid #d0d7de", backgroundColor: "#f6f8fa", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px" }}>👤</div>
                        )}
                        <div><div style={{ fontWeight: "600" }}>{u.name}</div><div style={{ fontSize: "12px", color: "#57606a" }}>{u.customId}</div></div></div></td>
                  ...
                  <tr key={t.id} style={{ backgroundColor: "white", borderBottom: "1px solid #e1e4e8" }}>
                    <td style={styles.td}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        {getPhotoUrl(t.userId) ? (
                          <img src={getPhotoUrl(t.userId)} style={{ width: "24px", height: "24px", borderRadius: "50%" }} onError={(e) => e.target.style.display = 'none'} />
                        ) : (
                          <div style={{ width: "24px", height: "24px", borderRadius: "50%", backgroundColor: "#f6f8fa", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px" }}>👤</div>
                        )}
                        <b>{t.userName}</b>
                      </div>
                    </td>
                ...
                <>
                  <div style={styles.teamHeader}>
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      ...
                      <button onClick={() => handleDeleteTeam(selectedTeam._id)} style={styles.dangerBtn}>Delete</button>
                    </div>
                  </div>
                  {teamViewMode === "tasks" ? (
                    <>
        ...
        {/* --- VIEW: MESSAGES --- */}
        {currentView === "chat" && (
          <Messaging currentUser={loggedInUser} />
        )}
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(400px, 1fr))", gap: "24px" }}>
              ...
              <div style={{ backgroundColor: "white", border: "1px solid #d0d7de", borderRadius: "6px", padding: "24px", textAlign: "center" }}>
                {getPhotoUrl(selectedUser._id) ? (
                  <img src={getPhotoUrl(selectedUser._id)} style={{ width: "120px", height: "120px", borderRadius: "50%", border: "1px solid #d0d7de", marginBottom: "16px" }} onError={(e) => e.target.style.display = 'none'} />
                ) : (
                  <div style={{ width: "120px", height: "120px", borderRadius: "50%", border: "1px solid #d0d7de", marginBottom: "16px", backgroundColor: "#f6f8fa", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: "48px" }}>👤</div>
                )}
